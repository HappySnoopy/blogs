...
public class EnumBuster<E extends Enum<E>> {
    ...
    private final ReflectionFactory reflection = ReflectionFactory
    ...
    public E make(String value) {
        return this.make(value, 0, EnumBuster.EMPTY_CLASS_ARRAY,
            EnumBuster.EMPTY_OBJECT_ARRAY);
    }
    ...
    public E make(String value, int ordinal) {
        return this.make(value, ordinal, EnumBuster.EMPTY_CLASS_ARRAY,
            EnumBuster.EMPTY_OBJECT_ARRAY);
    }
     ...
     */
    public E make(String value, int ordinal, Class[] additionalTypes,
            Object[] additional) {
        try {
            this.undoStack.push(new Memento());
            ConstructorAccessor ca = this
                .findConstructorAccessor(additionalTypes, this.clazz);
            return this.constructEnum(this.clazz, ca, value, ordinal,
                additional);
        } catch (Exception e) {
        ...
        try {
            ...
            for (int i = 0; i < values.length; i++) {
                ...
                if (value.name().equals(e.name())) {
                    ...
                    Field valuesField = this.findValuesField();
                    ReflectionHelper.setStaticFinalField(valuesField,
                        newValues);
    ...
    private ConstructorAccessor findConstructorAccessor(
            Class[] additionalParameterTypes, Class<E> clazz)
    ...
    private E constructEnum(Class<E> clazz, ConstructorAccessor ca,
            String value, int ordinal, Object[] additional) throws Exception {
    ...
    private void setOrdinal(E e, int ordinal)
        ...
        Field ordinalField = Enum.class
            .getDeclaredField(EnumBuster.ORDINAL_FIELD);
    ...
    private Field findValuesField() throws NoSuchFieldException {
        ...
        Field valuesField = this.clazz
            .getDeclaredField(EnumBuster.VALUES_FIELD);
        ...
        try {
            for (Class switchUser : switchUsers) {
                ...
                while (true) {
                    try {
                        Class suspect = Class
                            .forName(String.format("%s$%d", name, ++i));
                        Field[] fields = suspect.getDeclaredFields();
                        for (Field field : fields) {
                            String fieldName = field.getName();
                            if (fieldName.startsWith("$SwitchMap$") && fieldName
                                .endsWith(this.clazz.getSimpleName())) {
    ...
    private void removeSwitchCase(int ordinal) {
        try {
            for (Field switchField : this.switchFields) {
                int[] switches = (int[]) switchField.get(null);
                int[] newSwitches = Arrays.copyOf(switches,
                    switches.length - 1);
    ...
    private class Memento {
        ...
        private Memento() {
            try {
                ...
                for (Field switchField : EnumBuster.this.switchFields) {
                    ...
                    this.savedSwitchFieldValues.put(switchField,
                        switchArray.clone());
                }
            } catch (Exception e) {
                throw new IllegalArgumentException("Could not create the class",
                    e);
            }
        }
        ...
        private void undo()
            ...
            Field[] constantEnumFields = EnumBuster.this.clazz
                .getDeclaredFields();
            ...
            for (Map.Entry<Field, int[]> entry : this.savedSwitchFieldValues
                .entrySet()) {
